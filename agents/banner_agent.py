...
UNSPLASH_MAP = {
    "coffee": "https://images.unsplash.com/photo-1497935586351-b67a49e012bf?w=1024&q=80",
    ...
    "wrap": "https://images.unsplash.com/photo-1626804475297-41609ea0eb4eb?w=1024&q=80",
    "maggi": "https://images.unsplash.com/photo-1612929633738-8fe44f7ec841?w=1024&q=80",
    ...
    "default": "https://images.unsplash.com/photo-1554118811-1e0d58224f24?w=1024&q=80"
}
...
def _get_background_image(brief: CampaignBrief) -> Image.Image:
    ...
    if not os.path.exists(img_path):
        try:
            r = requests.get(UNSPLASH_MAP[keyword], timeout=10)
...
def _draw_banner(width: int, height: int, brief: CampaignBrief,
    ...
    for y in range(height):
        ...
        draw_overlay.line([(0, y), (width, y)], fill=(bg_dark[0], bg_dark[1], bg_dark[2], min(255, opacity + 40)))
    ...
    _safe_text(draw, (width // 2, hdr_h // 2), "✦  SUTO CAFÉ  ·  SILIGURI  ✦", hdr_font, accent)
    ...
    card_draw.rectangle([card_margin, card_top, width - card_margin, card_bot], fill=(bg_dark[0], bg_dark[1], bg_dark[2], 200))
    ...
    title_lines = _wrap_text(draw, brief.theme_display.upper(), title_font, int(width * 0.70))
    line_gap = int(width * 0.06)
    for i, line in enumerate(title_lines[:2]):
        _safe_text(draw, (width // 2, title_y + i * line_gap), line, title_font, text_light)

    items_font = _get_font(max(14, int(width * 0.035)), bold=False)
    items_y = card_top + int((card_bot - card_top) * 0.75)
    _safe_text(draw, (width // 2, items_y), "  ·  ".join(brief.hero_items[:3]), items_font, accent)
    ...
    _safe_text(draw, (width // 2, cta_top + (height - cta_top) // 2), f"→  {(brief.cta or 'Visit us today!')[:50]}  ←", cta_font, text_dark)

    try:
        draw.text((8, height - 16), f"[PoC] {label}", font=_get_font(max(10, int(width * 0.02))), fill=text_dark)
    except Exception:
        pass
